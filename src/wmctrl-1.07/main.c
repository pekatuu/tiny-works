...
static int client_msg(Display *disp, Window win, char *msg, 
        ...
        unsigned long data4);
static int print_active_window (Display *disp);
...
int main (int argc, char **argv) { /* {{{ */
    ...
    while ((opt = getopt(argc, argv, "AXFGVvhlupidmxa:r:s:c:t:w:k:o:n:g:e:b:N:I:T:R:")) != -1) {
        missing_option = 0;
        switch (opt) {
            case 'X':
                options.mark_active_window = 1;
                break;
            case 'F':
...
static int print_active_window (Display *disp) {
  Window active_window = get_active_window(disp);
  
  gchar *title_utf8 = get_window_title(disp, active_window); /* UTF8 */
  gchar *title_out = get_output_str(title_utf8, TRUE);
  gchar *client_machine;
  gchar *class_out = get_window_class(disp, active_window); /* UTF8 */
  unsigned long *pid;
  unsigned long *desktop;
  int x, y, junkx, junky;
  ...
  if ((client_machine = get_property(disp, active_window,
  ...
  /* desktop ID */
  if ((desktop = (unsigned long *)get_property(disp, active_window,
					       XA_CARDINAL, "_NET_WM_DESKTOP", NULL)) == NULL) {
    desktop = (unsigned long *)get_property(disp, active_window,
					    XA_CARDINAL, "_WIN_WORKSPACE", NULL);
  }

  /* client machine */
  client_machine = get_property(disp, active_window,
				XA_STRING, "WM_CLIENT_MACHINE", NULL);
       
  /* pid */
  pid = (unsigned long *)get_property(disp, active_window,
				      XA_CARDINAL, "_NET_WM_PID", NULL);

  /* geometry */
  XGetGeometry (disp, active_window, &junkroot, &junkx, &junky,
		&wwidth, &wheight, &bw, &depth);
  XTranslateCoordinates (disp, active_window, junkroot, junkx, junky,
			 &x, &y, &junkroot);
      
  /* special desktop ID -1 means "all desktops", so we 
     have to convert the desktop value to signed long */
  printf("0x%.8lx %2ld", active_window, 
...
static int list_windows (Display *disp) {/*{{{*/
    ...
    for (i = 0; i < client_list_size / sizeof(Window); i++) {
        ...
        /* print mark */
	if (options.mark_active_window) {
	  printf("%c ", active_window == client_list[i] ? 'X': ' ');
	}
